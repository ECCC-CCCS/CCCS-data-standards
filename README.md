...
### Relation avec les besoins de l'utilisateur
...
3. Décrit un accord pour que les développeurs de données fournissent au besoin un soutien et/ou une formation au personnel du centre d'aide CCSC pour répondre aux demandes de données/méthodes ; (*Recommandé*).
...
Les données doivent être :
1. Versionnées à l'aide d'un protocole de suivi cohérent (par exemple [Gestion sémantique de version (semver)](semver.org/lang/fr/) qui identifie les révisions majeures et mineures des données, pour les versions de données hébergées au CCCS et ailleurs.  Les versions de données énumérées doivent être associées aux [éléments suggérés par l'Organisation météorologique mondiale (OMM)](https://library.wmo.int/doc_num.php?explnum_id=7867) suivants: (*Obligatoire*)
...
4. Couvrir au minimum : (*Obligatoire*)
   ...
   2. Pour les données issues de l'observation : devraient couvrir au moins 30 ans et couvrir une période qui est applicable aux besoins identifiés des utilisateurs ;
...
Les données elles-mêmes, ou les ensembles de données ou la documentation à l'appui, doivent permettre au personnel et aux utilisateurs du CCCS de comprendre les sources d'incertitude.  L'"incertitude" peut prendre différentes formes selon les données en question.  En fonction de l'ensemble des données, les sources d'incertitude qui devraient être abordées pourraient inclure :
...
Les données doivent être associées à une documentation certifiante :
